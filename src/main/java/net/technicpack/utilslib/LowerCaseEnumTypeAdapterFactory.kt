...
class LowerCaseEnumTypeAdapterFactory constructor() : TypeAdapterFactory {
    public override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T> {
        val rawType: Class<*> = type.getRawType()
        if (!rawType.isEnum()) {
            return null
        }
        val lowercaseToConstant: MutableMap<String, T> = HashMap()
        for (constant: Any in rawType.getEnumConstants()) {
            lowercaseToConstant.put(toLowercase(constant), constant as T)
        }
        return object : TypeAdapter<T?>() {
            @Throws(IOException::class)
            public override fun write(out: JsonWriter, value: T?) {
                if (value == null) out.nullValue() else out.value(toLowercase(value))
            }

            @Throws(IOException::class)
            public override fun read(reader: JsonReader): T? {
