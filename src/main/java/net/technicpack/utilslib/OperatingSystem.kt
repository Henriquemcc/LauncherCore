...
enum class OperatingSystem(private override val name: String, private val aliases: Array<String?>) {
    LINUX("linux", arrayOf<String?>("linux", "unix")), WINDOWS("windows", arrayOf<String?>("win")), OSX(
        "osx",
        arrayOf<String?>("mac")
    ),
    UNKNOWN("unknown", arrayOfNulls<String>(0));

    fun getUserDirectoryForApp(appName: String): File {
        val userHome: String = System.getProperty("user.home", ".")
        when (this) {
            LINUX -> return File(userHome, "." + appName + "/")
            WINDOWS -> {
                val applicationData: String? = System.getenv("APPDATA")
                if (applicationData != null) {
                    return File(applicationData, "." + appName + "/")
                } else {
                    return File(userHome, "." + appName + "/")
                }
            }
            OSX -> return File(userHome, "Library/Application Support/" + appName)
            UNKNOWN -> return File(userHome, appName + "/")
            else -> return File(userHome, appName + "/")
        }
    }
    ...
    companion object {
        ...
        fun getJavaDir(): String {
            ...
            val path: String = System.getProperty("java.home") + separator + "bin" + separator
        ...
        fun getOperatingSystem(): OperatingSystem? {
            ...
            for (operatingSystem: OperatingSystem in values()) {
                for (alias: String? in operatingSystem.getAliases()) {
                    if (osName.contains((alias)!!)) {
                        Companion.operatingSystem = operatingSystem
                        return operatingSystem
                    }
                }
            }
