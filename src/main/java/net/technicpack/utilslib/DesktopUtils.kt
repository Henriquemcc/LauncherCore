...
object DesktopUtils {
    fun browseUrl(url: String) {
        try {
            if (url.startsWith("mailto:")) Desktop.getDesktop().mail(URI(url)) else Desktop.getDesktop()
                .browse(URI(url))
    ...
    fun open(file: File) {
        object : SwingWorker<Void?, Void?>() {
            ...
            override fun doInBackground(): Void? {
                ...
                var asciiUri: String = file.toURI().toASCIIString()
                Utils.getLogger().info("Using " + asciiUri)
                if (asciiUri.startsWith("file:") && !asciiUri.startsWith("file://")) asciiUri =
                    asciiUri.replaceFirst("file:".toRegex(), "file://")
                Utils.getLogger().info("Intermediary path " + asciiUri)
                try {
                    Desktop.getDesktop().open(File(URI(asciiUri).getPath()))
                } catch (ex: Exception) {
                    ex.printStackTrace()
                }
                return null
