...
object SHA1Utils {
    fun getSHA1(file: File?): String? {
        try {
            ...
            val sha1: String = DigestUtils.sha1Hex(fis)
            fis.close()
            return sha1
    ...
    fun checkSHA1(file: File?, sha1: String): Boolean {
        return checkSHA1(sha1, getSHA1(file))
    }

    fun checkSHA1(sha1: String, otherSha1: String?): Boolean {
        return sha1.equals(otherSha1, ignoreCase = true)
    }
}