...
object ZipUtils {
    fun checkLaunchDirectory(dir: File): Boolean {
        if (!dir.isDirectory()) {
            return false
        }
        if (dir.list().size == 0) {
     ...
     */
    fun checkEmpty(dir: File): Boolean {
        return dir.isDirectory() && dir.list().size == 0
    }

    @Throws(IOException::class, InterruptedException::class)
    fun extractFile(zip: File, output: File?, fileName: String?): Boolean {
        ...
        try {
            zipFile = ZipFile(zip)
            val entry: ZipArchiveEntry? = zipFile.getEntry(fileName)
            if (entry == null) {
                Utils.getLogger().log(Level.WARNING, "File " + fileName + " not found in " + zip.getAbsolutePath())
                return false
            }
            val outputFile: File = File(output, entry.getName())
    ...
    @Throws(IOException::class, InterruptedException::class)
    private fun unzipEntry(zipFile: ZipFile, entry: ZipArchiveEntry, outputFile: File) {
        val buffer: ByteArray = ByteArray(2048)
        val inputStream: BufferedInputStream = BufferedInputStream(zipFile.getInputStream(entry))
        val outputStream: BufferedOutputStream = BufferedOutputStream(FileOutputStream(outputFile))
        try {
            var length: Int
    ...
    @Throws(IOException::class, InterruptedException::class)
    fun unzipFile(zip: File, output: File?, fileFilter: IZipFileFilter?, listener: DownloadListener?) {
        if (!zip.exists()) {
        ...
        val zipFile: ZipFile = ZipFile(zip)
        val entries: ArrayList<ZipArchiveEntry> = Collections.list(zipFile.getEntries())
        ...
        if (size == 0) {
            ...
            zipFile.close()
            throw ZipException("Zip file is empty: " + zip.getAbsolutePath())
        }
        var progress: Int = 1
        try {
            for (entry: ZipArchiveEntry in entries) {
                if (Thread.interrupted()) throw InterruptedException()
                if (!entry.getName()
                        .contains("../") && (fileFilter == null || fileFilter.shouldExtract(entry.getName()))
                ) {
                    val outputFile: File = File(output, entry.getName())
                    if (outputFile.exists() && outputFile.isFile() && !outputFile.canWrite()) {
                        if (outputFile.delete()) {
                            Utils.getLogger()
                                .log(Level.INFO, "Deleted non-writable file " + outputFile.getAbsolutePath())
                        } else {
                            throw IOException("Failed to delete non-writable file " + outputFile.getAbsolutePath())
                        }
                    }
                    if (outputFile.getParentFile() != null) {
                        outputFile.getParentFile().mkdirs()
                ...
                if (listener != null) {
                    val totalProgress: Float = progress.toFloat() / size.toFloat()
