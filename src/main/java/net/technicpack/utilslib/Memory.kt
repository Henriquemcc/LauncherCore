...
class Memory private constructor(memory: Int, text: String, option: Int) {
    var memory: Long
    var text: String
    var option: Int

    init {
        this.memory = memory.toLong()
        this.text = text
        this.option = option
    }

    fun getMemoryMB(): Long {
        return memory
    }
    ...
    companion object {
        val memoryOptions: Array<Memory> = arrayOf(
            (Memory(512, "512 MB", 1)),
            (Memory(768, "768 MB", 2)),
            (Memory(1024, "1 GB", 0)),
            (Memory(1536, "1.5 GB", 3)),
            (Memory(2048, "2 GB", 4)),
            (Memory(2560, "2.5 GB", 19)),
            (Memory(3072, "3 GB", 5)),
            (Memory(3584, "3.5 GB", 20)),
            (Memory(4096, "4 GB", 6)),
            (Memory(5120, "5 GB", 7)),
            (Memory(6144, "6 GB", 8)),
            (Memory(7168, "7 GB", 9)),
            (Memory(8192, "8 GB", 10)),
            (Memory(9216, "9 GB", 11)),
            (Memory(10240, "10 GB", 12)),
            (Memory(11264, "11 GB", 13)),
            (Memory(12288, "12 GB", 14)),
            (Memory(13312, "13 GB", 15)),
            (Memory(14336, "14 GB", 16)),
            (Memory(15360, "15 GB", 17)),
            (Memory(16384, "16 GB", 18))
        )
        val DEFAULT_MEM: Memory = memoryOptions.get(2)
        val MAX_32_BIT_MEMORY: Int = 1024
        fun is64Bit(): Boolean {
            val architecture: String = System.getProperty("sun.arch.data.model", "32")
        ...
        fun getPhysicalMemory(): Long {
            ...
            try {
                val osInfo: OperatingSystemMXBean = ManagementFactory.getOperatingSystemMXBean()
                if (osInfo is com.sun.management.OperatingSystemMXBean) {
                    maxMemory = osInfo.getTotalPhysicalMemorySize() / 1024 / 1024
                }
            } catch (t: Throwable) {
            }
        ...
        fun getAvailableMemory(is64Bit: Boolean): Long {
            val physical: Long = getPhysicalMemory()
            if (!is64Bit && physical > MAX_32_BIT_MEMORY) return MAX_32_BIT_MEMORY.toLong()
            return physical
        }

        fun getClosestAvailableMemory(memory: Memory?): Memory? {
            return getClosestAvailableMemory(memory, is64Bit())
        }

        fun getClosestAvailableMemory(memory: Memory?, is64Bit: Boolean): Memory? {
            val available: Long = getAvailableMemory(is64Bit)
            if (memory!!.getMemoryMB() <= available) return memory
            var bestMemory: Memory = memoryOptions.get(0)
            for (option: Memory in memoryOptions) {
                if (option.getMemoryMB() <= available && option.getMemoryMB() > bestMemory.getMemoryMB()) bestMemory =
                    option
            }
            return bestMemory
        }

        fun getMemoryFromId(id: Int): Memory {
            for (m: Memory in memoryOptions) {
