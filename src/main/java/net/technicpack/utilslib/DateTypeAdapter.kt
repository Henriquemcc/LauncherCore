...
class DateTypeAdapter constructor() : JsonDeserializer<Date>, JsonSerializer<Date> {
    ...
    init {
        ...
        iso8601Format = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssZ")
    }

    @Throws(JsonParseException::class)
    public override fun deserialize(json: JsonElement, typeOfT: Type, context: JsonDeserializationContext): Date {
        ...
        throw IllegalArgumentException(javaClass.toString() + " cannot deserialize to " + typeOfT)
    }
    ...
    private fun deserializeToDate(json: JsonElement): Date {
        synchronized(enUsFormat, {
            try {
                ...
                } catch (localParseException1: ParseException) {
                    try {
                        var cleaned: String = json.getAsString().replace("Z", "+00:00")
                        cleaned = cleaned.substring(0, 22) + cleaned.substring(23)
                        return iso8601Format.parse(cleaned)
                    } catch (e: Exception) {
                        throw JsonSyntaxException("Invalid date: " + json.getAsString(), e)
                    }
                }
            }
        })
    ...
    public override fun serialize(src: Date, typeOfSrc: Type, context: JsonSerializationContext): JsonElement {
        synchronized(enUsFormat, { return JsonPrimitive(serializeToString(src)) })
    }

    private fun serializeToString(date: Date): String {
        synchronized(enUsFormat, {
            val result: String = iso8601Format.format(date)
