...
class MavenListenerAdapter(private val listener: DownloadListener) : TransferListener {
    private var total: Long = 0
    private val completedAmounts: MutableMap<String, Long> = HashMap()
    @Throws(TransferCancelledException::class)
    override fun transferInitiated(transferEvent: TransferEvent) {
    }

    @Throws(TransferCancelledException::class)
    override fun transferStarted(transferEvent: TransferEvent) {
        completedAmounts[transferEvent.resource.resourceName] = 0L
        total += transferEvent.resource.contentLength
        updateValue(transferEvent.resource, transferEvent.transferredBytes)
    }

    @Throws(TransferCancelledException::class)
    override fun transferProgressed(transferEvent: TransferEvent) {
        updateValue(transferEvent.resource, transferEvent.transferredBytes)
    }

    @Throws(TransferCancelledException::class)
    override fun transferCorrupted(transferEvent: TransferEvent) {
    }

    override fun transferSucceeded(transferEvent: TransferEvent) {
        updateValue(transferEvent.resource, transferEvent.resource.contentLength)
    }

    override fun transferFailed(transferEvent: TransferEvent) {}
    private fun updateValue(resource: TransferResource, bytes: Long) {
        completedAmounts[resource.resourceName] = bytes
        var completed: Long = 0
        for (value in completedAmounts.values) {
            completed += value
        }
        val total = total.toFloat()
        val progress = completed.toFloat()
        val percent = 100.0f * progress / total
