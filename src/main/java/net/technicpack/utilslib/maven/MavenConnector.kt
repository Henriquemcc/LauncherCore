...
class MavenConnector(directories: LauncherDirectories, vararg defaultRepositories: String?) {
    private val defaultRepositories: MutableList<RemoteRepository>
    ...
    init {
        ...
        val localRepository = LocalRepository(directories.cacheDirectory)
        session.localRepositoryManager = system.newLocalRepositoryManager(session, localRepository)
        this.defaultRepositories = ArrayList(defaultRepositories.size / 2)
        for (i in 0 until defaultRepositories.size / 2) {
            val key = defaultRepositories[i * 2]
            val url = defaultRepositories[i * 2 + 1]
            this.defaultRepositories.add(RemoteRepository.Builder(key, "default", url).build())
        }
    }

    fun attemptLibraryDownload(name: String?, listener: DownloadListener?): Boolean {
        return attemptLibraryDownload(name, null, listener)
    }

    @JvmOverloads
    fun attemptLibraryDownload(name: String?, url: String? = null, listener: DownloadListener? = null): Boolean {
        val dependency = Dependency(DefaultArtifact(name), "compile")
        val collectRequest = CollectRequest()
        collectRequest.root = dependency
        if (url != null) collectRequest.addRepository(RemoteRepository.Builder("targetted", "default", url).build())
        for (repo in defaultRepositories) {
        ...
        return try {
            ...
            if (listener != null) session.transferListener = MavenListenerAdapter(listener)
            system.resolveDependencies(session, dependencyRequest)
            true
        } catch (ex: DependencyCollectionException) {
            false
