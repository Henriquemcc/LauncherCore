...
open class RestObject {
    ...
    companion object {
        ...
        fun <T : RestObject?> postRestObject(restObject: Class<T>?, url: String): T {
            try {
                ...
                conn.inputStream.use { stream ->
                    val data = IOUtils.toString(stream, StandardCharsets.UTF_8)
                    val result = gson.fromJson(data, restObject)
                        ?: throw RestfulAPIException("Unable to access URL [$url]")
                    if (result.hasError()) {
                        throw RestfulAPIException("Error in response: " + result.error)
                    }
                    return result
                }
            } catch (e: SocketTimeoutException) {
                throw RestfulAPIException("Timed out accessing URL [$url]", e)
            } catch (e: MalformedURLException) {
                throw RestfulAPIException("Invalid URL [$url]", e)
            } catch (e: JsonParseException) {
                throw RestfulAPIException("Error parsing response JSON at URL [$url]", e)
            } catch (e: IOException) {
                throw RestfulAPIException("Error accessing URL [$url]", e)
            }
        }

        @Throws(RestfulAPIException::class)
        fun <T> getRestArray(restObject: Class<T>?, url: String): List<T> {
            try {
                ...
                conn.getInputStream().use { stream ->
                    val data = IOUtils.toString(stream, StandardCharsets.UTF_8)
                    val response = gson.fromJson(data, JsonElement::class.java)
                    if (response == null || !response.isJsonArray) {
                        if (response!!.isJsonObject && response.asJsonObject.has("error")) throw RestfulAPIException("Error in response: " + response.asJsonObject["error"]) else throw RestfulAPIException(
                            "Unable to access URL [$url]"
                        )
                    }
                    val array = response.asJsonArray
                    val result: MutableList<T> = ArrayList(array.size())
                    for (element in array) {
                        if (element.isJsonObject) result.add(
                            gson.fromJson(
                                element.asJsonObject,
                                restObject
                            )
                        ) else result.add(
                            gson.fromJson(element.asString, restObject)
                        )
                    }
                    return result
                }
        ...
        @Throws(RestfulAPIException::class)
        fun <T : RestObject?> getRestObject(restObject: Class<T>?, url: String?): T {
            try {
                val conn = URL(url).openConnection()
                conn.setRequestProperty("User-Agent", TechnicConstants.getUserAgent())
                conn.connectTimeout = 15000
