...
class HttpPlatformApi(
    ...
    private val launcherBuild: String
) : IPlatformApi {
    ...
    override fun getPlatformPackInfo(packSlug: String?): PlatformPackInfo? {
        val url = getPlatformUri(packSlug)
        return RestObject.Companion.getRestObject<PlatformPackInfo>(PlatformPackInfo::class.java, url)
    }

    override fun incrementPackRuns(packSlug: String) {
        val url = platformUrl + "modpack/" + packSlug + "/stat/run?build=" + launcherBuild
        Utils.pingHttpURL(url, mirrorStore)
    }

    override fun incrementPackInstalls(packSlug: String?) {
        val url = platformUrl + "modpack/" + packSlug + "/stat/install?build=" + launcherBuild
        Utils.pingHttpURL(url, mirrorStore)
    }

    @Throws(RestfulAPIException::class)
    override fun getNews(): NewsData? {
        val url = platformUrl + "news?build=" + launcherBuild
        return RestObject.Companion.getRestObject<NewsData>(NewsData::class.java, url)
    }
}