...
class HttpPlatformSearchApi(private val rootUrl: String, private val launcherBuild: String) : IPlatformSearchApi {
    @Throws(RestfulAPIException::class)
    override fun getSearchResults(searchTerm: String): SearchResultsData {
        return try {
            val url = rootUrl + "search?build=" + launcherBuild + "&q=" + URLEncoder.encode(
                searchTerm.trim { it <= ' ' },
                "UTF-8"
            )
            RestObject.Companion.getRestObject<SearchResultsData>(SearchResultsData::class.java, url)
