...
open class PlatformPackInfoRepository(private val platform: IPlatformApi, private val solder: ISolderApi) :
    IAuthoritativePackSource {
    override fun getPackInfo(pack: InstalledPack?): PackInfo? {
        return getPlatformPackInfo(pack!!.name)
    }

    override fun getCompletePackInfo(pack: PackInfo?): PackInfo? {
        return getPlatformPackInfo(pack.getName())
    }

    protected fun getPlatformPackInfo(slug: String?): PackInfo? {
        return try {
            var info: PackInfo? = null
            val platformInfo = platform.getPlatformPackInfoForBulk(slug)
    ...
    protected fun getInfoFromPlatformInfo(platformInfo: PlatformPackInfo?): PackInfo? {
        return if (platformInfo != null && platformInfo.hasSolder()) {
            try {
                val solderPack = solder.getSolderPack(
                    platformInfo.solder,
                    platformInfo.name,
                    solder.getMirrorUrl(platformInfo.solder)
                )
                val solderInfo = solderPack.packInfoForBulk
                solderInfo?.let { CombinedPackInfo(it, platformInfo) } ?: platformInfo
            } catch (ex: RestfulAPIException) {
                ex.printStackTrace()
                platformInfo
            }
