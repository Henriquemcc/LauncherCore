...
class SinglePlatformSource(platformApi: IPlatformApi, solderApi: ISolderApi, private val slug: String) :
    PlatformPackInfoRepository(platformApi, solderApi), IPackSource {
    override fun getSourceName(): String {
        return "Platform pack with slug '" + slug + "'"
    }

    override fun getPublicPacks(): Collection<PackInfo?> {
        val packs = ArrayList<PackInfo?>(1)
        val info = getPlatformPackInfo(slug)
        if (info != null) {
            packs.add(info)
        }
        return packs
