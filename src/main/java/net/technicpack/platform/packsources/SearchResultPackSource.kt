...
class SearchResultPackSource(private val platformApi: IPlatformSearchApi, private val searchTerms: String) :
    IPackSource {
    private val resultPriorities: MutableMap<String?, Int> = HashMap()
    ...
    override fun getPublicPacks(): Collection<PackInfo?> {
        resultPriorities.clear()
        //Get results from server
        var results: SearchResultsData? = null
        results = try {
            platformApi.getSearchResults(searchTerms)
        } catch (ex: RestfulAPIException) {
            return emptySet<PackInfo>()
        }
        val resultPacks = ArrayList<PackInfo?>(results.getResults().size)
        var priority = 100
        for (result in results.getResults()) {
            resultPacks.add(SearchResultPackInfo(result))
            resultPriorities[result!!.slug] = priority--
        }
        return resultPacks
    }

    override fun getPriority(info: PackInfo?): Int {
        return if (resultPriorities.containsKey(info.getName())) resultPriorities[info.getName()]!! else 0
    }
}