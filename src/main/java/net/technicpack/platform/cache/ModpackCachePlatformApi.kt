...
class ModpackCachePlatformApi(
    private val innerApi: IPlatformApi,
    cacheInSeconds: Int,
    private val directories: LauncherDirectories
) : IPlatformApi {
    private val cache: Cache<String?, PlatformPackInfo>
    private val deadPacks: Cache<String?, Boolean>
    private val foreverCache: Cache<String?, PlatformPackInfo>
    ...
    override fun getPlatformPackInfoForBulk(packSlug: String?): PlatformPackInfo? {
        var info = foreverCache.getIfPresent(packSlug)
        if (info == null) {
    ...
    @Throws(RestfulAPIException::class)
    override fun getPlatformPackInfo(packSlug: String?): PlatformPackInfo? {
        var info = cache.getIfPresent(packSlug)
    ...
    protected fun getDeadPackInfo(packSlug: String?): PlatformPackInfo? {
        return try {
            val deadInfo = getPlatformPackInfoForBulk(packSlug)
            deadInfo?.setLocal()
            deadInfo
        } catch (ex: RestfulAPIException) {
            null
    ...
    private fun isDead(packSlug: String?): Boolean {
        val isDead = deadPacks.getIfPresent(packSlug)
        return if (isDead != null && isDead.toBoolean()) true else false
    }

    @Throws(RestfulAPIException::class)
    private fun pullAndCache(packSlug: String?): PlatformPackInfo? {
        var info: PlatformPackInfo? = null
        try {
    ...
    private fun loadForeverCache(packSlug: String?): PlatformPackInfo? {
        val cacheFile = File(File(File(directories.assetsDirectory, "packs"), packSlug), "cache.json")
        return if (!cacheFile.exists()) null else try {
            val packCache = FileUtils.readFileToString(cacheFile, Charset.forName("UTF-8"))
            val info = Utils.getGson()!!
                .fromJson(packCache, PlatformPackInfo::class.java)
            if (info != null) {
                foreverCache.put(packSlug, info)
            }
            info
    ...
    private fun saveForeverCache(info: PlatformPackInfo) {
        val cacheFile = File(File(File(directories.assetsDirectory, "packs"), info.name), "cache.json")
    ...
    override fun getPlatformUri(packSlug: String?): String? {
        return innerApi.getPlatformUri(packSlug)
    }

    override fun incrementPackRuns(packSlug: String) {
        innerApi.incrementPackRuns(packSlug)
    }

    override fun incrementPackInstalls(packSlug: String?) {
        innerApi.incrementPackInstalls(packSlug)
    }

    @Throws(RestfulAPIException::class)
    override fun getNews(): NewsData? {
        return innerApi.news
    }
}