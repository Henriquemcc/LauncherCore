...
class HttpUpdateStream constructor(private val baseUrl: String) : IUpdateStream {
    @Throws(RestfulAPIException::class)
    public override fun getStreamVersion(stream: String?): StreamVersion {
        return RestObject.Companion.getRestObject<StreamVersion>(
            StreamVersion::class.java,
            baseUrl + "version/" + stream
        )
    }
}