...
class MoveLauncherPackage constructor(
    ...
    public override fun runTask(queue: InstallTasksQueue<*>?) {
        val currentPath: String? = relauncher.getRunningPath()
        Utils.getLogger()
            .log(Level.INFO, "Moving running package from " + currentPath + " to " + launcher.getAbsolutePath())
        val source: File = File(currentPath)
        val dest: File = File(launcher.getAbsolutePath())
        if (!(source == dest)) {
            if (dest.exists()) {
                if (!dest.delete()) Utils.getLogger().log(Level.SEVERE, "Deletion of existing package failed!")
            }
            try {
