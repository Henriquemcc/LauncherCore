...
class DownloadUpdate constructor(
    url: String?,
    private val relauncher: Relauncher,
    private val postUpdateActions: Collection<IInstallTask>
) : DownloadFileTask(url, relauncher.getTempLauncher(), null, relauncher.getUpdateText()) {
    @Throws(IOException::class, InterruptedException::class)
    public override fun runTask(queue: InstallTasksQueue<*>) {
        super.runTask(queue)
        if (!relauncher.isUpdateOnly() && getDestination()!!.exists()) {
            for (task: IInstallTask? in postUpdateActions) {
                queue.addTask((task)!!)
            }
        }
