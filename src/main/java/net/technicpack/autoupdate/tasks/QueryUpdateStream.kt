...
class QueryUpdateStream constructor(
    private val description: String,
    private val updateStream: IUpdateStream,
    private val downloadTasks: ITasksQueue,
    private val directories: LauncherDirectories,
    private val relauncher: Relauncher,
    private val postDownloadTasks: Collection<IInstallTask>
) : IInstallTask {
    ...
    public override fun runTask(queue: InstallTasksQueue<*>?) {
        try {
            val version: StreamVersion? = updateStream.getStreamVersion(relauncher.getStreamName())
            if (version == null || version.getBuild() == 0) return
            for (resource: LauncherResource? in version.getResources()) {
                val verifier: IFileVerifier = MD5FileVerifier(resource!!.getMd5())
                val downloadFile: File =
                    File(File(directories.getAssetsDirectory(), "launcher"), resource.getFilename())
                if (!downloadFile.exists() || !verifier.isFileValid(downloadFile)) downloadTasks.addTask(
                    DownloadFileTask(
                        resource.getUrl(), downloadFile, verifier, resource.getFilename()
                    )
                )
            }
            if (version.getBuild() == relauncher.getCurrentBuild() || (relauncher.getStreamName()
                    .startsWith("beta") && version.getBuild() <= relauncher.getCurrentBuild())
            ) return
            var updateUrl: String? = null
            val runningPath: String? = relauncher.getRunningPath()
            if (runningPath == null) {
                throw DownloadException("Could not load a running path for currently-executing launcher.")
            }
            if (runningPath.endsWith(".exe")) updateUrl = version.getExeUrl() else updateUrl = version.getJarUrl()
            downloadTasks.addTask(DownloadUpdate(updateUrl, relauncher, postDownloadTasks))
