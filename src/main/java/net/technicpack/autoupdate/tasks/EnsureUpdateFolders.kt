...
class EnsureUpdateFolders constructor(
    ...
    public override fun runTask(queue: InstallTasksQueue<*>?) {
        val launcherAssets: File = File(directories.getAssetsDirectory(), "launcher")
        val patches: File = File(launcherAssets, "patches")
        if (!patches.exists()) patches.mkdirs()
    }
}