...
class StreamVersion constructor() : RestObject() {
    private val build: Int = 0
    private val url: StreamUrls? = null
    private val resources: List<LauncherResource> = ArrayList()
    fun getBuild(): Int {
        return build
    }

    fun getExeUrl(): String? {
        return url!!.getExeUrl()
    }

    fun getJarUrl(): String? {
        return url!!.getJarUrl()
    }

    fun getResources(): List<LauncherResource> {
        return resources
    }
}