...
class CachedSolderApi(
    private val directories: LauncherDirectories,
    private val innerApi: ISolderApi,
    ...
    private inner class CacheTuple(private val root: String?, private val slug: String?, private val url: String?) {
        override fun equals(obj: Any?): Boolean {
            ...
            if (slug != obj.slug) return false
            return if (url != obj.url) false else true
        }
    ...
    private val packs: Cache<CacheTuple, ISolderPackApi>
    ...
    override fun getSolderPack(solderRoot: String?, modpackSlug: String?, mirrorUrl: String?): ISolderPackApi {
        val tuple = CacheTuple(solderRoot, modpackSlug, mirrorUrl)
        var pack = packs.getIfPresent(tuple)
        if (pack == null) {
            pack = CachedSolderPackApi(
                directories,
    ...
    override fun internalGetPublicSolderPacks(
        ...
        if (Seconds.secondsBetween(lastSolderPull, DateTime.now()).isLessThan(
                Seconds.seconds(
                    cacheInSeconds
                )
            )
        ...
        return if (Seconds.secondsBetween(lastSolderPull, DateTime.now()).isLessThan(
                Seconds.seconds(
                    cacheInSeconds / 10
                )
            )
        ) ArrayList(0) else try {
