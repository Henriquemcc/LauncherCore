...
class HttpSolderPackApi(
    private val baseUrl: String?,
    private val modpackSlug: String?,
    ...
    init {
        if (mirrorUrl == null) throw RestfulAPIException("A mirror URL could not be retrieved from '" + baseUrl + "modpack'")
    }
    ...
    override val packInfo: SolderPackInfo?
        get() {
            val packUrl = baseUrl + "modpack/" + modpackSlug + "?cid=" + clientId
            val info: SolderPackInfo =
                RestObject.Companion.getRestObject<SolderPackInfo>(SolderPackInfo::class.java, packUrl)
            info.solder = this
            return info
        }
    ...
    override fun getPackBuild(build: String): Modpack? {
        try {
            val url = baseUrl + "modpack/" + modpackSlug + "/" + build + "?cid=" + clientId
            val pack: Modpack = RestObject.Companion.getRestObject<Modpack>(Modpack::class.java, url)
            if (pack != null) {
                return pack
            }
        } catch (e: RestfulAPIException) {
            throw BuildInaccessibleException(modpackSlug, build, e)
        }
        throw BuildInaccessibleException(modpackSlug, build)
    }
}