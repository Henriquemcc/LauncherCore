...
class HttpSolderApi(private val clientId: String, private val userModel: UserModel<*>) : ISolderApi {
    private val mirrorUrls: MutableMap<String?, String?> = HashMap()
    @Throws(RestfulAPIException::class)
    override fun getSolderPack(solderRoot: String?, modpackSlug: String?, mirrorUrl: String?): ISolderPackApi {
        return HttpSolderPackApi(solderRoot, modpackSlug, clientId, userModel.currentUser!!.displayName, mirrorUrl)
    }

    @Throws(RestfulAPIException::class)
    override fun getPublicSolderPacks(solderRoot: String): Collection<SolderPackInfo?>? {
        return internalGetPublicSolderPacks(solderRoot, this)
    }

    @Throws(RestfulAPIException::class)
    override fun getMirrorUrl(solderRoot: String?): String? {
        if (!mirrorUrls.containsKey(solderRoot)) {
            val allPacksUrl = solderRoot + "modpack"
            val solder: Solder = RestObject.Companion.getRestObject<Solder>(Solder::class.java, allPacksUrl)
            mirrorUrls[solderRoot] = solder.mirrorUrl
        }
        return mirrorUrls[solderRoot]
    }

    @Throws(RestfulAPIException::class)
    override fun internalGetPublicSolderPacks(
        solderRoot: String,
        packFactory: ISolderApi
    ): Collection<SolderPackInfo?>? {
        val allPackApis = LinkedList<SolderPackInfo?>()
        val allPacksUrl = solderRoot + "modpack?include=full&cid=" + clientId
        val technic: FullModpacks =
            RestObject.Companion.getRestObject<FullModpacks>(FullModpacks::class.java, allPacksUrl)
        for (info in technic.modpacks.values) {
            val solder = packFactory.getSolderPack(solderRoot, info.name, technic.mirrorUrl)
            info.solder = solder
