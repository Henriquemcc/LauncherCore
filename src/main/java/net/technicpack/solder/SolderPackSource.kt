...
class SolderPackSource(private val baseUrl: String, private val solder: ISolderApi) : IPackSource {
    override val sourceName: String
        get() = "Public packs for solder $baseUrl"
    override val publicPacks: Collection<PackInfo?>
        get() {
            val returnValue = LinkedList<PackInfo?>()
            try {
                val packs = solder.getPublicSolderPacks(baseUrl)
                for (info in packs!!) {
    ...
    override fun getPriority(pack: PackInfo?): Int {
        return -1
    }
}