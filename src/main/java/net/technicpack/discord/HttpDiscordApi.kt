...
class HttpDiscordApi(private val url: String) : IDiscordApi {
    override fun retrieveServer(modpack: ModpackModel?, serverId: String, callback: IDiscordCallback) {
        object : SwingWorker<Server?, Server?>() {
            public override fun doInBackground(): Server? {
                val guildUrl = url + "servers/" + serverId + "/widget.json"
                try {
                    return RestObject.Companion.getRestObject<Server>(Server::class.java, guildUrl)
