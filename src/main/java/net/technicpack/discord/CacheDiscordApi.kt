...
class CacheDiscordApi(private val innerApi: IDiscordApi, cacheLength: Int, deadCacheLength: Int) : IDiscordApi {
    private val cache: Cache<String, Server>
    private val deadCache: Cache<String, Boolean>

    init {
        cache = CacheBuilder.newBuilder()
            .concurrencyLevel(4)
            .maximumSize(300)
            .expireAfterWrite(cacheLength.toLong(), TimeUnit.SECONDS)
            .build()
        deadCache = CacheBuilder.newBuilder()
            .concurrencyLevel(4)
            .maximumSize(300)
            .expireAfterWrite(deadCacheLength.toLong(), TimeUnit.SECONDS)
            .build()
    }

    override fun retrieveServer(modpack: ModpackModel?, serverId: String, callback: IDiscordCallback) {
        val deadCacheValue = deadCache.getIfPresent(serverId)
        if (deadCacheValue != null && deadCacheValue) return
        val cacheValue = cache.getIfPresent(serverId)
        if (cacheValue != null) {
            if (deadCacheValue == null) deadCache.put(serverId, false)
