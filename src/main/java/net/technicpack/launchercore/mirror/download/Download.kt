...
class Download constructor(private val url: URL, private val name: String?, private val outPath: String) : Runnable {
    ...
    private val timeoutLock: Any = Any()
    private var isTimedOut: Boolean = false
    fun getProgress(): Float {
        return (downloaded.toFloat() / size) * 100
    }
    ...
    public override fun run() {
        ...
        try {
            var conn: HttpURLConnection? = Utils.openHttpConnection(url)
            var response: Int = conn!!.getResponseCode()
            var responseFamily: Int = response / 100
            if (responseFamily == 3) {
                val redirUrlText: String? = conn.getHeaderField("Location")
                if (redirUrlText != null && !redirUrlText.isEmpty()) {
                    var redirectUrl: URL? = null
                    try {
                        redirectUrl = URL(redirUrlText)
            ...
            if (responseFamily != 2) {
                throw DownloadException("The server issued a " + response + " response code.")
            }
            val `in`: InputStream = getConnectionInputStream(conn)
            size = conn.getContentLength().toLong()
    ...
    protected fun getConnectionInputStream(urlconnection: URLConnection?): InputStream {
        val `is`: AtomicReference<InputStream?> = AtomicReference()
        var j: Int = 0
        while ((j < 3) && (`is`.get() == null)) {
            val stream: StreamThread = StreamThread(urlconnection, `is`)
            stream.start()
        ...
        if (`is`.get() == null) {
            throw DownloadException("Unable to download file from " + urlconnection!!.getURL())
        }
        return BufferedInputStream(`is`.get())
    }

    private fun stateChanged() {
        if (listener != null) listener!!.stateChanged(name, getProgress())
    }

    private fun timeout() {
        synchronized(timeoutLock, { isTimedOut = true })
    }
    ...
    private class StreamThread constructor(
        ...
        val permDenied: AtomicBoolean = AtomicBoolean(false)
        public override fun run() {
            try {
                `is`.set(urlconnection!!.getInputStream())
            } catch (e: SocketException) {
                if (e.message.equals("Permission denied: connect", ignoreCase = true)) {
                    permDenied.set(true)
                }
    ...
    inner class MonitorThread constructor(downloadThread: Thread, rbc: ReadableByteChannel?) :
        ...
        public override fun run() {
            while (!isInterrupted()) {
                val diff: Long = outFile!!.length() - downloaded
