...
class JsonWebSecureMirror constructor(private val baseUrl: String, private val downloadHost: String) : ISecureMirror {
    public override fun getDownloadHost(): String {
        return downloadHost
    }

    public override fun validate(req: ValidateRequest): ValidateResponse {
        val constructedUrl: String = baseUrl + "validate?a=" + req.getAccessToken() + "&c=" + req.getClientToken()
        try {
            return RestObject.Companion.getRestObject<ValidateResponse>(ValidateResponse::class.java, constructedUrl)
        } catch (ex: RestfulAPIException) {
            Utils.getLogger().log(Level.SEVERE, ex.message, ex)
            return ValidateResponse(ex.message)
        }
    }
}