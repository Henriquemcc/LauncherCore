...
class RotatingFileHandler constructor(private val logFile: String) : StreamHandler() {
    ...
    init {
        ...
        try {
            setOutputStream(FileOutputStream(filename, true))
    ...
    private fun calculateFilename(): String {
        return logFile.replace("%D", date.format(Date()))
    }

    @Synchronized
    public override fun flush() {
        if (!(filename == calculateFilename())) {
            filename = calculateFilename()
            try {
                close()
                setOutputStream(FileOutputStream(filename, true))
