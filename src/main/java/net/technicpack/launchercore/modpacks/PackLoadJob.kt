...
class PackLoadJob constructor(
    private val directories: LauncherDirectories,
    private val packRepository: IInstalledPackRepository,
    private val authoritativeSource: IAuthoritativePackSource,
    private val packSources: Collection<IPackSource>?,
    private val container: IModpackContainer,
    private val tagBuilder: IModpackTagBuilder?,
    private val doLoadRepository: Boolean
) : Runnable {
    private val processedModpacks: MutableMap<String?, ModpackModel> = HashMap()
    ...
    public override fun run() {
        ...
        val threads: MutableCollection<Thread> = ArrayList(threadCount)
        if (doLoadRepository) {
            for (packName: String? in packRepository.getPackNames()) {
                val pack: InstalledPack? = packRepository.getInstalledPacks().get(packName)
                addPackThreadSafe(pack, null, -1)
            }
        }
        if (packSources != null) {
            for (packSource: IPackSource in packSources) {
                val packSourceThread: Thread = object : Thread(packSource.getSourceName() + " Loading Thread") {
                    public override fun run() {
    ...
    protected fun refreshCompleteThreadSafe() {
        EventQueue.invokeLater(object : Runnable {
            public override fun run() {
    ...
    protected fun addPackThreadSafe(pack: InstalledPack?, packInfo: PackInfo?, priority: Int) {
        EventQueue.invokeLater(object : Runnable {
            public override fun run() {
    ...
    protected fun addPack(pack: InstalledPack?, packInfo: PackInfo?, priority: Int) {
        ...
        if (processedModpacks.containsKey(name)) {
            ...
            modpack = ModpackModel(pack, packInfo, packRepository, directories)
            modpack.updatePriority(priority)
            if (packInfo == null) modpack.setIsPlatform(false)
            processedModpacks.put(name, modpack)
        }
        if ((modpack!!.getInstalledPack() == null) && !doLoadRepository && packRepository.getInstalledPacks()
                .containsKey(
                    modpack.getName()
                )
        ) {
        ...
        var fillDataMethod: Runnable? = null
        if (modpack.getPackInfo() == null) {
            fillDataMethod = object : Runnable {
                public override fun run() {
                    val completeInfo: PackInfo? = authoritativeSource.getPackInfo(pack)
        ...
        if (fillDataMethod != null) {
            val thread: Thread = Thread(fillDataMethod)
            thread.start()
        }
        if (modpack != null && tagBuilder != null) modpack.updateTags(tagBuilder)
        if (newModpackModel) container.addModpackToContainer(modpack) else container.replaceModpackInContainer(modpack)
    }
}