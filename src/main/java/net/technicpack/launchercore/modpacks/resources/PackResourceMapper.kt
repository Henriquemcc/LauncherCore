...
class PackResourceMapper constructor(
    private val directories: LauncherDirectories,
    private val defaultImage: BufferedImage,
    private val resourceType: IModpackResourceType
) : IImageMapper<ModpackModel> {
    public override fun shouldDownloadImage(imageKey: ModpackModel): Boolean {
        ...
        if (res.getMd5() == null || res.getMd5().isEmpty()) return true
    ...
    public override fun getImageLocation(imageKey: ModpackModel): File {
        val assets: File = File(directories.getAssetsDirectory(), "packs")
        val packs: File = File(assets, imageKey.getName())
        return File(packs, resourceType.getImageName())
    }

    public override fun getDefaultImage(): BufferedImage {
        return defaultImage
    }
}