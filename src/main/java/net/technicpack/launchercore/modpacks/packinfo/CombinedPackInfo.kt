...
class CombinedPackInfo constructor(private val solderPackInfo: PackInfo?, private val platformPackInfo: PackInfo?) :
    PackInfo {
    ...
    public override fun getLatest(): String? {
        if (solderPackInfo != null) return solderPackInfo.getLatest()
        if (platformPackInfo != null) return platformPackInfo.getRecommended()
        return null
    }

    public override fun getBuilds(): List<String?>? {
        if (solderPackInfo != null) return solderPackInfo.getBuilds()
        if (platformPackInfo != null) return platformPackInfo.getBuilds()
        return ArrayList(0)
    }

    public override fun shouldForceDirectory(): Boolean {
        if (platformPackInfo != null) return platformPackInfo.shouldForceDirectory()
        return false
    }

    public override fun getFeed(): ArrayList<FeedItem?>? {
    ...
    public override fun getRuns(): Int? {
        if (platformPackInfo != null) return platformPackInfo.getRuns()
        return null
    }

    public override fun getDownloads(): Int? {
        if (platformPackInfo != null) return platformPackInfo.getDownloads()
        return null
    }

    public override fun getLikes(): Int? {
        if (platformPackInfo != null) return platformPackInfo.getLikes()
        return null
    }

    public override fun isServerPack(): Boolean {
        if (platformPackInfo != null) return platformPackInfo.isServerPack()
        return false
    }

    public override fun isOfficial(): Boolean {
        if (platformPackInfo != null) return platformPackInfo.isOfficial()
        return false
    }

    @Throws(BuildInaccessibleException::class)
    public override fun getModpack(build: String): Modpack? {
    ...
    public override fun isComplete(): Boolean {
        return (platformPackInfo != null)
    }

    public override fun isLocal(): Boolean {
        if (platformPackInfo == null || platformPackInfo.isLocal()) return true
        if (solderPackInfo == null || solderPackInfo.isLocal()) return true
        return false
    }
}