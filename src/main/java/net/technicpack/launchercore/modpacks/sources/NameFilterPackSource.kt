...
class NameFilterPackSource constructor(private val baseModpacks: MemoryModpackContainer, filter: String) : IPackSource {
    private val filterTerms: String

    init {
        filterTerms = filter.uppercase(Locale.getDefault())
    }

    public override fun getSourceName(): String {
        return "Installed packs filtered by '" + filterTerms + "'"
    }

    public override fun getPublicPacks(): Collection<PackInfo?> {
        val info: LinkedList<PackInfo?> = LinkedList()
        for (modpack: ModpackModel? in baseModpacks.getModpacks()) {
    ...
    public override fun getPriority(info: PackInfo?): Int {
        for (modpack: ModpackModel? in baseModpacks.getModpacks()) {
            if ((modpack!!.getName() == info.getName())) return modpack.getPriority()
        }
        return 0
    }
}