...
class MemoryModpackContainer constructor() : IModpackContainer {
    private val modpacks: MutableMap<String?, ModpackModel?> = HashMap()
    private val passthroughContainers: MutableMap<IModpackContainer, IModpackContainer> = ConcurrentHashMap()
    public override fun clear() {
        modpacks.clear()
        for (container: IModpackContainer in passthroughContainers.values) container.clear()
    }

    public override fun addModpackToContainer(modpack: ModpackModel?) {
        modpacks.put(modpack!!.getName(), modpack)
        for (container: IModpackContainer in passthroughContainers.values) container.addModpackToContainer(modpack)
    }

    public override fun replaceModpackInContainer(modpack: ModpackModel?) {
        if (modpacks.containsKey(modpack!!.getName())) modpacks.put(modpack.getName(), modpack)
        for (container: IModpackContainer in passthroughContainers.values) container.replaceModpackInContainer(modpack)
    }

    public override fun refreshComplete() {
        for (container: IModpackContainer in passthroughContainers.values) container.refreshComplete()
    }

    fun addPassthroughContainer(container: IModpackContainer) {
        passthroughContainers.put(container, container)
    }

    fun removePassthroughContainer(container: IModpackContainer) {
        passthroughContainers.remove(container)
    }

    fun getModpacks(): Collection<ModpackModel?> {
