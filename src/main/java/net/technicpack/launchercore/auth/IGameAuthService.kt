...
open interface IGameAuthService<UserData> {
    fun createClearedUser(username: String?, response: IAuthResponse?): UserData
    fun createOfflineUser(displayName: String?): UserData

    @Throws(AuthenticationNetworkFailureException::class)
    fun requestRefresh(user: UserData): IAuthResponse?

    @Throws(AuthenticationNetworkFailureException::class)
    fun requestLogin(username: String?, password: String?, data: String?): IAuthResponse?
}