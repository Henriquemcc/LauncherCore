...
class UserModel<UserType : IUserType?> constructor(
    userStore: IUserStore<*>,
    gameAuthService: IGameAuthService<UserType>
) {
    private var mCurrentUser: UserType? = null
    private val mAuthListeners: MutableList<IAuthListener<*>> = LinkedList()
    private val mUserStore: IUserStore<UserType>
    private val gameAuthService: IGameAuthService<UserType>
    ...
    fun addAuthListener(listener: IAuthListener<UserType>) {
    ...
    fun attemptUserRefresh(user: UserType): AuthError? {
        ...
        if (response == null) {
            ...
            mUserStore.removeUser(user!!.getUsername())
            return AuthError(response.getError(), response.getErrorMessage())
        } else {
    ...
    fun attemptInitialLogin(username: String?, password: String?): AuthError? {
        try {
            val response: IAuthResponse? = gameAuthService.requestLogin(username, password, getClientToken())
            if (response == null) {
                return AuthError("Auth Error", "Invalid credentials. Invalid username or password.")
            } else if (response.getError() != null) {
                return AuthError(response.getError(), response.getErrorMessage())
            } else {
                //Create an online user with the received data
                val clearedUser: UserType? = gameAuthService.createClearedUser(username, response)
                setCurrentUser(clearedUser)
                return null
            }
        } catch (ex: AuthenticationNetworkFailureException) {
            ex.printStackTrace()
            return AuthError(
    ...
    fun initAuth() {
        val user: UserType? = getLastUser()
        if (user != null) {
            try {
                val error: AuthError? = attemptUserRefresh(user)
                if (error != null) setCurrentUser(null)
            } catch (ex: AuthenticationNetworkFailureException) {
    ...
    fun getUsers(): Collection<UserType?>? {
        return mUserStore.getSavedUsers()
    }

    fun getLastUser(): UserType? {
        return mUserStore.getUser(mUserStore.getLastUser())
    }

    fun getUser(username: String?): UserType? {
    ...
    fun removeUser(user: UserType) {
        mUserStore.removeUser(user!!.getUsername())
    }

    fun setLastUser(user: UserType) {
    ...
    inner class AuthError constructor(private val mError: String?, private val mErrorDescription: String?) {
        fun getError(): String? {
            return mError
        }
