...
class UnzipFileTask constructor(
    private val zipFile: File,
    private val destination: File?,
    private val filter: IZipFileFilter?
) : ListenerTask() {
    public override fun getTaskDescription(): String? {
        return "Unzipping " + zipFile.getName()
    }
    ...
    public override fun runTask(queue: InstallTasksQueue<*>?) {
        super.runTask(queue)
        if (!zipFile.exists()) {
            throw ZipException("Attempting to extract file " + zipFile.getName() + ", but it did not exist.")
        }
        ...
        try {
            ...
            ex.printStackTrace()
            throw ZipException("Error extracting file " + zipFile.getName() + ".")
        }
    }
}