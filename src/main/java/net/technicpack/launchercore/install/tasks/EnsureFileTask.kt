...
open class EnsureFileTask<TaskQueue : ITasksQueue?> @JvmOverloads constructor(
    private val cacheLocation: File,
    private val fileVerifier: IFileVerifier?,
    private val zipExtractLocation: File?,
    private val sourceUrl: String?,
    private val friendlyFileName: String,
    private val downloadTaskQueue: TaskQueue,
    private val copyTaskQueue: TaskQueue,
    private val filter: IZipFileFilter? = null
) : IInstallTask {
    constructor(
        fileLocation: File,
        fileVerifier: IFileVerifier?,
        zipExtractLocation: File?,
        sourceUrl: String?,
    ...
    public override fun getTaskDescription(): String? {
        return "Verifying " + cacheLocation.getName()
    }
    ...
    public override fun runTask(queue: InstallTasksQueue<*>?) {
        if (zipExtractLocation != null) unzipFile(copyTaskQueue, cacheLocation, zipExtractLocation, filter)
        if (sourceUrl != null && (!cacheLocation.exists() || (fileVerifier != null && !fileVerifier.isFileValid(
                cacheLocation
            )))
        ) downloadFile(downloadTaskQueue, sourceUrl, cacheLocation, fileVerifier, friendlyFileName)
    }

    open fun unzipFile(taskQueue: TaskQueue, zipLocation: File, targetLocation: File?, filter: IZipFileFilter?) {
        taskQueue!!.addNextTask(UnzipFileTask(zipLocation, targetLocation, filter))
    }
    ...
    open fun downloadFile(
        ...
        sourceUrl: String?,
        targetLocation: File?,
        verifier: IFileVerifier?,
        fileName: String?
    ) {
        taskQueue!!.addNextTask(DownloadFileTask(sourceUrl, targetLocation, verifier, fileName))
    }
}