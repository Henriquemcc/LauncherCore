...
class Version {
    ...
    constructor() {}
    constructor(version: String?, legacy: Boolean) {
    ...
    fun save(saveDirectory: File?) {
        val version: File = File(saveDirectory, "version")
        val json: String = Utils.getGson()!!.toJson(this)
        try {
    ...
    companion object {
        fun load(version: File): Version? {
            if (!version.exists()) {
                Utils.getLogger()
                    .log(Level.WARNING, "Unable to load version from " + version + " because it does not exist.")
                return null
            }
            try {
                val json: String = FileUtils.readFileToString(version, Charset.forName("UTF-8"))
                return Utils.getGson()!!
                    .fromJson(json, Version::class.java)
