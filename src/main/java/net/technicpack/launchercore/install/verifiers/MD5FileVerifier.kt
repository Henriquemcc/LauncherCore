...
class MD5FileVerifier constructor(private val md5Hash: String?) : IFileVerifier {
    public override fun isFileValid(file: File?): Boolean {
        if (md5Hash == null || md5Hash.isEmpty()) return false
        val resultMD5: String? = MD5Utils.getMD5(file)
        Utils.getLogger().info("Expected MD5: " + md5Hash + " Calculated MD5: " + resultMD5)
        return (md5Hash.equals(resultMD5, ignoreCase = true))
    }
}