...
class ValidJsonFileVerifier constructor(private val validatingGson: Gson) : IFileVerifier {
    public override fun isFileValid(file: File?): Boolean {
        try {
            val json: String = FileUtils.readFileToString(file, Charset.forName("UTF-8"))
            val obj: JsonObject? = validatingGson.fromJson(json, JsonObject::class.java)
            return (obj != null)
        } catch (ex: Exception) {
            Utils.getLogger().log(
                Level.SEVERE,
                "An exception was raised while verifying " + file!!.getAbsolutePath() + "- this probably just means the file is invalid, in which case this is not an error:",
                ex
            )
        }
        return false
