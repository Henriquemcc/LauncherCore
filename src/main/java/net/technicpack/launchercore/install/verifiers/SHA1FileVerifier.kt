...
class SHA1FileVerifier constructor(private val sha1Hash: String?) : IFileVerifier {
    public override fun isFileValid(file: File?): Boolean {
        if (sha1Hash == null || sha1Hash.isEmpty()) return false
        val resultSha1: String? = SHA1Utils.getSHA1(file)
        Utils.getLogger().info("Expected SHA1: " + sha1Hash + " Calculated SHA1: " + resultSha1)
        return (sha1Hash.equals(resultSha1, ignoreCase = true))
    }
}