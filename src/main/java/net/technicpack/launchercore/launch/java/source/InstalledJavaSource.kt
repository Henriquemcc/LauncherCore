 ...
 */
class InstalledJavaSource constructor() : IVersionSource {
    public override fun enumerateVersions(repository: JavaVersionRepository) {
        if (OperatingSystem.Companion.getOperatingSystem() == OperatingSystem.WINDOWS) (WinRegistryJavaSource()).enumerateVersions(
            repository
        ) else if (OperatingSystem.Companion.getOperatingSystem() == OperatingSystem.OSX) (MacInstalledJavaSource()).enumerateVersions(
            repository
        )
    }
}