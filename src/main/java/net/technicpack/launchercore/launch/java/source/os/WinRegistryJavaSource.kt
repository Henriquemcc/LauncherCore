 ...
 *
 */
class WinRegistryJavaSource constructor() : IVersionSource {
    ...
    fun enumerateRegistryForBitness(repository: JavaVersionRepository, bitness: String) {
        val output: String? = Utils.getProcessOutput(
            "reg",
            "query",
        ...
        for (line: String in output.split("\\r?\\n").toTypedArray()) {
            ...
            var typeIndex: Int = line.indexOf("REG_SZ")
            if (typeIndex < 0) continue
            typeIndex += "REG_SZ".length
            val path: String = line.substring(typeIndex).trim({ it <= ' ' })
            repository.addVersion(FileBasedJavaVersion(File(path + File.separator + "bin" + File.separator + "javaw.exe")))
        }
    }
}