 ...
 */
class FileJavaSource protected constructor(@field:Transient private var loadedFile: File?) : IVersionSource {
    private val versions: MutableList<FileBasedJavaVersion> = ArrayList()
    protected fun setLoadedFile(loadedFile: File?) {
        this.loadedFile = loadedFile
    }

    public override fun enumerateVersions(repository: JavaVersionRepository) {
        for (version: FileBasedJavaVersion in versions) {
            repository.addVersion(version)
        }
    }

    fun addVersion(version: FileBasedJavaVersion) {
        versions.add(version)
        save()
    }

    fun save() {
        val data: String = Utils.getGson()!!.toJson(this)
        try {
    ...
    companion object {
        fun load(file: File?): FileJavaSource {
            if (file == null || !file.exists()) return FileJavaSource(file)
            try {
                val sourceText: String = FileUtils.readFileToString(file)
                val source: FileJavaSource = Utils.getGson()!!
                    .fromJson(sourceText, FileJavaSource::class.java)
                source.setLoadedFile(file)
                return source
            } catch (ex: IOException) {
                ex.printStackTrace()
                return FileJavaSource(file)
            }
        }
    }
}