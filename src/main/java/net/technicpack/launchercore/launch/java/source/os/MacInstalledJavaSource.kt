...
class MacInstalledJavaSource constructor() : IVersionSource {
    public override fun enumerateVersions(repository: JavaVersionRepository) {
        repository.addVersion(FileBasedJavaVersion(File(getMacJava("1.6"))))
        repository.addVersion(FileBasedJavaVersion(File(getMacJava("1.7"))))
        repository.addVersion(FileBasedJavaVersion(File(getMacJava("1.8"))))
        repository.addVersion(FileBasedJavaVersion(File("/Library/Internet Plug-Ins/JavaAppletPlugin.plugin/Contents/Home/bin/java")))
    }

    protected fun getMacJava(versionNumber: String?): String {
        val path: String? = Utils.getProcessOutput("/usr/libexec/java_home", "-v", versionNumber)
