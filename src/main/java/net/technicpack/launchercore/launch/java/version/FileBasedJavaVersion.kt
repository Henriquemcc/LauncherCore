 ...
 */
class FileBasedJavaVersion : IJavaVersion {
    @Transient
    private var haveQueriedVersion: Boolean = false
    ...
    @Transient
    private var isOpenJDK: Boolean = false

    @Transient
    private var javaPath: File? = null
    private var filePath: String? = null

    constructor() {}
    constructor(javaPath: File) {
        this.javaPath = javaPath
        filePath = javaPath.getAbsolutePath()
    }

    public override fun getVersionNumber(): String? {
    ...
    public override fun equals(obj: Any?): Boolean {
        ...
        if (!(obj is FileBasedJavaVersion)) return false
        return (is64Bit() == obj.is64Bit() && (getVersionNumber() == obj.getVersionNumber()))
    }

    public override fun hashCode(): Int {
        var hash: Int = getVersionNumber().hashCode()
        hash = (hash * 31) + (if (is64Bit()) 1 else 0)
        return hash
    }

    public override fun is64Bit(): Boolean {
        if (!haveQueriedVersion) {
            verify()
        }
        return is64Bit
    ...
    public override fun isOpenJDK(): Boolean {
        if (!haveQueriedVersion) {
            verify()
        }
        return isOpenJDK
    ...
    public override fun getJavaPath(): File? {
        if ((javaPath == null) && (filePath != null) && !filePath!!.isEmpty()) javaPath = File(filePath)
        return javaPath
    }
     ...
     */
    public override fun verify(): Boolean {
        if (getJavaPath() == null || !getJavaPath()!!.exists()) return false
        if (!haveQueriedVersion) {
     ...
     * the executable.
     */
    protected fun getVersionNumberFromJava(): String? {
        var versionJavaExec: String? = filePath
        ...
        val data: String? = Utils.getProcessOutput(versionJavaExec, "-version")
        if (data == null) return null
        is64Bit = data.contains("64-Bit")
        val versionPattern: Pattern = Pattern.compile("(java|openjdk) version \"(.+?)\"")
        val versionMatcher: Matcher = versionPattern.matcher(data)
        if (!versionMatcher.lookingAt()) return null
        isOpenJDK = (versionMatcher.group(1) == "openjdk")
