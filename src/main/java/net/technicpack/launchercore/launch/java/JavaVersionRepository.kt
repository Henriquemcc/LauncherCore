 ...
 */
class JavaVersionRepository constructor() {
    private val loadedVersions: MutableMap<File?, IJavaVersion> = HashMap()
    private val versionCache: MutableCollection<IJavaVersion> = LinkedList()
    private var selectedVersion: IJavaVersion?

    init {
        val version: IJavaVersion = CurrentJavaVersion()
        selectedVersion = version
        loadedVersions.put(null, version)
    }

    fun addVersion(version: IJavaVersion) {
        if (!version.verify()) return
        val path: File? = version.getJavaPath()
        val versionText: String? = version.getVersionNumber()
        if (versionCache.contains(version)) return
        loadedVersions.put(path, version)
        versionCache.add(version)
        if (selectedVersion == null) selectedVersion = version
    }

    fun getBest64BitVersion(): IJavaVersion? {
        var bestVersion: IJavaVersion? = null
        for (version: IJavaVersion in loadedVersions.values) {
            if (version.is64Bit()) {
                if (bestVersion == null || bestVersion.getVersionNumber() == null) {
                    bestVersion = version
                    continue
                }
                if (version.getVersionNumber() == null) continue
                if (version.getVersionNumber()!!.compareTo(bestVersion.getVersionNumber()!!) > 0) bestVersion = version
            }
        }
        return bestVersion
    }

    fun getVersions(): Collection<IJavaVersion> {
        return versionCache
    }

    fun getSelectedVersion(): IJavaVersion? {
        return selectedVersion
    }

    fun selectVersion(version: String?, is64Bit: Boolean) {
        selectedVersion = getVersion(version, is64Bit)
    }

    fun getVersion(version: String?, is64Bit: Boolean): IJavaVersion? {
        if ((version == null) || version.isEmpty() || (version == "default")) {
            return loadedVersions.get(null)
        } else if ((version == "64bit")) {
            var best64BitVersion: IJavaVersion? = getBest64BitVersion()
            if (best64BitVersion == null) best64BitVersion = loadedVersions.get(null)
            return best64BitVersion
        } else {
            for (checkVersion: IJavaVersion in versionCache) {
                if ((version == checkVersion.getVersionNumber()) && is64Bit == checkVersion.is64Bit()) return checkVersion
            }
            var specifiedVersion: IJavaVersion? = loadedVersions.get(File(version))
    ...
    fun getSelectedPath(): String {
        if (selectedVersion == null || selectedVersion!!.getJavaPath() == null) return OperatingSystem.Companion.getJavaDir()
