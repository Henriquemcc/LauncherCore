...
class ImageJob<T> constructor(protected var mapper: IImageMapper<T>, protected var store: IImageStore<T>) {
    private var lastJobData: T? = null
    protected var canRetry: Boolean = true
    private val imageReference: AtomicReference<BufferedImage?>
    private val jobListeners: MutableCollection<IImageJobListener<T>> = LinkedList()

    init {
        imageReference = AtomicReference()
    ...
    fun removeJobListener(listener: IImageJobListener<*>?) {
        synchronized(jobListeners, { jobListeners.remove(listener) })
    }
    ...
    protected fun setImage(image: BufferedImage?) {
        canRetry = false
    ...
    fun start(jobData: T) {
        lastJobData = jobData
        val imageThread: Thread = object : Thread("Image Download: " + store.getJobKey(jobData)) {
            public override fun run() {
                try {
                    val imageLocation: File? = mapper.getImageLocation(jobData)
                    var existingImage: BufferedImage? = null
