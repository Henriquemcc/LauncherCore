...
class CrafatarFaceImageStore constructor(private val mBaseUrl: String, private val mirrorStore: MirrorStore) :
    IImageStore<IUserType> {
    public override fun canDownloadImage(user: IUserType, location: File?): Boolean {
        return true
    }

    public override fun downloadImage(user: IUserType, location: File) {
        try {
            mirrorStore.downloadFile(
                mBaseUrl + "avatars/" + user.getId() + "?size=100&helm",
                user.getDisplayName(),
    ...
    public override fun getJobKey(key: IUserType): String {
        return "user-face-" + key.getDisplayName()
    }

    public override fun canRetry(key: IUserType): Boolean {
        return false
    }
}