...
class WebAvatarImageStore constructor(private val mirrorStore: MirrorStore) : IImageStore<AuthorshipInfo> {
    public override fun canDownloadImage(key: AuthorshipInfo, target: File?): Boolean {
        return true
    }

    public override fun downloadImage(key: AuthorshipInfo, target: File) {
        try {
    ...
    public override fun getJobKey(key: AuthorshipInfo): String {
        return "user-avatar-" + key.getUser()
    }

    public override fun canRetry(key: AuthorshipInfo): Boolean {
        return false
    }
}